...
> **Project standard — Python tooling:** Run tests with `uv run pytest` (uv-managed deps). Lint & format with `ruff` via pre-commit (`ruff-check`, `ruff-format`); coverage is measured with `pytest-cov`.
